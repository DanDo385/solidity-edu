 ...
 */
contract DeployMerkleAllowlist is Script {
    function run() external {
        ...
        MerkleAllowlistSolution nft = new MerkleAllowlistSolution(merkleRoot);

        console.log("Deployed MerkleAllowlistSolution at:", address(nft));
 ...
 *
 * ----------------------------------------------------------------------------
 * TYPESCRIPT (Node.js with ethers v6 and merkletreejs)
 * ----------------------------------------------------------------------------
 *
 * ```typescript
 ...
 *
 * // 2. Create leaf nodes
 * const leaves: Buffer[] = allowlist.map(address =>
 ...
 *    $ npx ts-node generate-merkle.ts
 ...
 *      CONTRACT_ADDRESS \
 *      src/solution/MerkleAllowlistSolution.sol:MerkleAllowlistSolution
 *
 * ----------------------------------------------------------------------------
