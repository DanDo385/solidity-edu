 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * 1. INFLATION ATTACKS EXPLOIT FIRST DEPOSITOR
 *    ✅ Attacker deposits 1 wei, gets 1 share
 *    ✅ Attacker donates large amount to vault
 *    ✅ Now 1 share = huge value
 *    ✅ Victim deposits, gets 0 shares (rounds down!)
 *    ✅ Real-world: Multiple exploits before mitigations
 *
 * 2. VIRTUAL SHARES PREVENT ATTACKS
 *    ✅ Mint dead shares to address(0) on first deposit
 *    ✅ Prevents 1:1 ratio manipulation
 *    ✅ Standard pattern in OpenZeppelin ERC4626
 *    ✅ Offset of 1e18 is common
 *
 * 3. MINIMUM DEPOSIT REQUIREMENTS HELP
 *    ✅ Require minimum deposit amount
 *    ✅ Prevents dust attacks
 *    ✅ Simple but effective
 *    ✅ Trade-off: Higher barrier to entry
 *
 * 4. DONATION ATTACKS BREAK ACCOUNTING
 *    ✅ Direct transfers to vault increase totalAssets
 *    ✅ If using balanceOf(), accounting breaks
 *    ✅ Track deposits internally instead
 *    ✅ CONNECTION TO PROJECT 11: Donation attack mitigation!
 *
 * 5. ROUNDING CAN CAUSE ZERO SHARES
 *    ✅ shares = (assets * totalSupply) / totalAssets
 *    ✅ If totalAssets >> totalSupply, shares rounds to 0
 *    ✅ Victim loses deposit
 *    ✅ Always require shares > 0
 *
 * 6. MULTIPLE MITIGATIONS PROVIDE DEFENSE
 *    ✅ Virtual shares (prevent manipulation)
 *    ✅ Minimum deposit (prevent dust)
 *    ✅ Internal tracking (prevent donation attacks)
 *    ✅ Require shares > 0 (prevent zero shares)
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                        COMMON MISTAKES
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * ❌ Not minting virtual shares (inflation attack!)
 * ❌ Using balanceOf() for accounting (donation attack!)
 * ❌ Not requiring minimum deposit (dust attacks!)
 * ❌ Not checking shares > 0 (zero shares!)
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                          NEXT STEPS
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * • Study OpenZeppelin ERC4626 virtual shares implementation
 * • Review real-world inflation attack exploits
 * • Learn about first depositor protection patterns
 * • Move to Project 45 to learn about multi-asset vaults
 */
