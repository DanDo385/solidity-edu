 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * 1. ERC-721A SAVES MASSIVE GAS FOR BATCH MINTING
 *    ✅ Mint 5 tokens: ~175k gas (vs ~750k standard)
 *    ✅ Savings: ~77% reduction!
 *    ✅ O(1) storage writes regardless of quantity
 *    ✅ Real-world: Saved Azuki users millions in gas
 *
 * 2. OWNERSHIP INFERENCE IS THE KEY OPTIMIZATION
 *    ✅ Only store ownership for first token in batch
 *    ✅ Scan backwards to find batch owner
 *    ✅ O(n) lookup but saves O(n) storage writes
 *    ✅ Trade-off: Slightly slower lookups, massive gas savings
 *
 * 3. STORAGE PACKING REDUCES COSTS
 *    ✅ Pack multiple values into single slot
 *    ✅ TokenOwnership: address + timestamp + burned flag
 *    ✅ AddressData: balance + minted + burned + aux
 *    ✅ Saves storage slots and gas
 *
 * 4. SEQUENTIAL TOKEN IDs ARE REQUIRED
 *    ✅ Tokens must be minted sequentially (0, 1, 2, ...)
 *    ✅ Enables ownership inference
 *    ✅ Not suitable for random or custom IDs
 *    ✅ Perfect for public mints
 *
 * 5. USE CASES FOR ERC-721A
 *    ✅ Public mints (users buy multiple NFTs)
 *    ✅ Airdrops/batch distributions
 *    ✅ Large collections (1000+ items)
 *    ❌ Single-mint-only collections (use standard ERC-721)
 *    ❌ Non-sequential IDs (use standard ERC-721)
 *
 * 6. GAS SAVINGS SCALE WITH BATCH SIZE
 *    ✅ Minting 1 token: Similar gas to standard ERC-721
 *    ✅ Minting 5 tokens: ~77% savings
 *    ✅ Minting 10 tokens: ~85% savings
 *    ✅ Larger batches = better savings
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                        COMMON MISTAKES
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * ❌ Using ERC-721A for single-mint collections (no benefit!)
 * ❌ Trying to use non-sequential IDs (breaks ownership inference!)
 * ❌ Not understanding ownership inference (confusing lookups)
 * ❌ Assuming all operations are faster (lookups are slower)
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                          NEXT STEPS
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * • Study Azuki's ERC-721A implementation
 * • Explore batch minting patterns
 * • Learn about storage optimization techniques
 * • Move to Project 26 to learn about ERC-1155 multi-token standard
 */
