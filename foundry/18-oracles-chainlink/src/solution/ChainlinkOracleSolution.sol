 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * 1. ORACLES BRIDGE BLOCKCHAIN AND REAL-WORLD DATA
 *    ✅ Blockchains are deterministic and isolated
 *    ✅ Oracles provide external data (prices, weather, etc.)
 *    ✅ Chainlink is decentralized oracle network
 *    ✅ Real-world: Like a news reporter bringing outside information
 *
 * 2. ALWAYS VALIDATE ORACLE DATA
 *    ✅ Check staleness (updatedAt threshold)
 *    ✅ Check round completeness (answeredInRound)
 *    ✅ Check price bounds (minPrice, maxPrice)
 *    ✅ Circuit breaker for extreme movements
 *
 * 3. CIRCUIT BREAKER PATTERN PROTECTS AGAINST MANIPULATION
 *    ✅ Detects abnormal price swings
 *    ✅ Pauses operations when triggered
 *    ✅ Requires manual reset
 *    ✅ Prevents flash loan attacks
 *
 * 4. PRICE FEED DECIMALS MATTER
 *    ✅ Chainlink feeds use 8 decimals typically
 *    ✅ Must scale calculations correctly
 *    ✅ Check decimals() function
 *    ✅ Handle different decimal formats
 *
 * 5. MULTIPLE SAFETY LAYERS ARE CRITICAL
 *    ✅ Staleness check (time-based)
 *    ✅ Completeness check (round-based)
 *    ✅ Bounds check (min/max price)
 *    ✅ Deviation check (circuit breaker)
 *
 * 6. ORACLE MANIPULATION IS A REAL RISK
 *    ✅ Flash loans can manipulate DEX prices
 *    ✅ Single oracle = single point of failure
 *    ✅ Use multiple sources when possible
 *    ✅ Consider TWAP for smoothing
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                        COMMON MISTAKES
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * ❌ Not checking staleness (using old prices!)
 * ❌ Not checking round completeness (incomplete rounds!)
 * ❌ Not handling negative prices (int256 can be negative!)
 * ❌ Not scaling decimals correctly
 * ❌ Trusting single oracle source
 * ❌ Not implementing circuit breaker
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                          NEXT STEPS
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * • Study TWAP (Time-Weighted Average Price) patterns
 * • Explore multiple oracle aggregation
 * • Learn about Chainlink VRF for randomness
 * • Move to Project 19 to learn about signed messages
 */
