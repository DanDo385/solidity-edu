 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * 1. ACCESS CONTROL IS FOUNDATIONAL SECURITY
 *    ✅ Determines who can perform privileged operations
 *    ✅ Missing or weak access control = complete compromise
 *    ✅ Real-world: Many exploits start with access control bugs
 *    ✅ CONNECTION TO PROJECT 04: Modifiers and restrictions!
 *
 * 2. UNINITIALIZED OWNER IS CRITICAL VULNERABILITY
 *    ✅ Owner never set = address(0)
 *    ✅ First caller of setOwner becomes owner
 *    ✅ Complete contract takeover possible
 *    ✅ Fix: Initialize in constructor or initializer
 *
 * 3. MISSING MODIFIERS ARE EASY TO MISS
 *    ✅ Function looks secure but missing onlyOwner
 *    ✅ Anyone can call privileged functions
 *    ✅ Code review must check every function
 *    ✅ Use linters to catch missing modifiers
 *
 * 4. tx.origin vs msg.sender MATTERS
 *    ✅ msg.sender: Immediate caller (could be contract)
 *    ✅ tx.origin: Original transaction sender (EOA)
 *    ✅ Using tx.origin breaks contract composability
 *    ✅ Real-world: Phishing attacks exploit this
 *
 * 5. OPENZEPPELIN ACCESS CONTROL IS PRODUCTION-READY
 *    ✅ Role-based access control (RBAC)
 *    ✅ Granular permissions per role
 *    ✅ Battle-tested and audited
 *    ✅ Use instead of custom implementations
 *
 * 6. INITIALIZATION PATTERNS ARE CRITICAL
 *    ✅ Constructors: For non-proxy contracts
 *    ✅ Initializers: For proxy contracts
 *    ✅ Protect from re-initialization
 *    ✅ Always validate owner != address(0)
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                        COMMON MISTAKES
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * ❌ Not initializing owner (anyone can become owner!)
 * ❌ Missing onlyOwner modifier (privileged functions exposed!)
 * ❌ Using tx.origin instead of msg.sender (breaks composability!)
 * ❌ Not protecting initializer functions (re-initialization!)
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                          NEXT STEPS
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * • Study OpenZeppelin AccessControl library
 * • Learn about role-based access control patterns
 * • Review real-world access control exploits
 * • Move to Project 37 to learn about gas DoS attacks
 */
