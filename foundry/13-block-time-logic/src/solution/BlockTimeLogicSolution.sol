 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * 1. block.timestamp vs block.number
 *    ✅ block.timestamp: Seconds since epoch, miner can manipulate ±15s
 *    ✅ block.number: Sequential block number, more predictable
 *    ✅ Use timestamp for long periods (hours/days)
 *    ✅ Use block.number for short periods or precise ordering
 *
 * 2. MINER MANIPULATION IS REAL
 *    ✅ Miners can manipulate timestamp by ±15 seconds
 *    ✅ For short periods (<15s), timestamp is unreliable
 *    ✅ For long periods (hours/days), manipulation is negligible
 *    ✅ Never use block properties for randomness!
 *
 * 3. ALWAYS USE >= NOT == FOR TIME COMPARISONS
 *    ✅ require(block.timestamp >= unlockTime) - Correct!
 *    ❌ require(block.timestamp == unlockTime) - Fragile!
 *    ✅ Time won't hit exact seconds reliably
 *    ✅ >= ensures action happens after deadline
 *
 ...
 *    ✅ Common pattern for public functions
 *
 * 5. COOLDOWN PERIODS ENABLE TWO-STEP PROCESSES
 *    ✅ User initiates action (starts cooldown)
 *    ✅ User executes action after cooldown
 *    ✅ Prevents accidental or malicious actions
 *    ✅ Common for critical operations
 *
 * 6. VESTING REQUIRES LINEAR CALCULATIONS
 *    ✅ Calculate elapsed time vs total duration
 *    ✅ Release proportional tokens
 *    ✅ Handle edge cases (before start, after end)
 *    ✅ Real-world: Employee stock options, token unlocks
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                        COMMON MISTAKES
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * ❌ Using block.timestamp for short periods (<15 seconds)
 * ❌ Using == instead of >= for time comparisons
 * ❌ Using block properties for randomness (miners can manipulate!)
 * ❌ Assuming exact block times (varies by network)
 * ❌ Not accounting for miner manipulation
 * ❌ Using blockhash for randomness (only last 256 blocks!)
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                          NEXT STEPS
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * • Study Chainlink VRF for secure randomness
 * • Explore time-locked governance patterns
 * • Learn about vesting contract implementations
 * • Move to Project 14 to learn about ABI encoding
 */
