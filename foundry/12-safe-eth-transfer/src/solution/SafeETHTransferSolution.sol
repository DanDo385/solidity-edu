 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * 1. PULL PAYMENTS ARE SAFER THAN PUSH PAYMENTS
 *    ✅ Pull: Users withdraw their own funds (self-service)
 *    ✅ Push: Contract sends funds to users (DoS risk)
 *    ✅ Pull prevents one malicious user from blocking others
 *    ✅ Real-world: ATM (pull) vs mail delivery (push)
 *
 * 2. PUSH PAYMENTS HAVE MULTIPLE RISKS
 *    ✅ DoS attack: One malicious recipient blocks all payments
 *    ✅ Unbounded gas: Loops may exceed block gas limit
 *    ✅ Reentrancy: External calls before state updates
 *    ✅ Always prefer pull pattern for distributions!
 *
 * 3. CHECKS-EFFECTS-INTERACTIONS PATTERN IS CRITICAL
 *    ✅ Checks: Validate conditions first
 *    ✅ Effects: Update state second
 *    ✅ Interactions: External calls last
 *    ✅ Prevents reentrancy attacks
 *
 * 4. USE .call{value:}() NOT .transfer() OR .send()
 *    ✅ .call{value:}(): Forwards all gas, works with smart contracts
 *    ❌ .transfer(): Only 2,300 gas, breaks with smart contracts
 *    ❌ .send(): Same problems as .transfer()
 *    ✅ Always check return value!
 *
 * 5. REENTRANCYGUARD PROVIDES DEFENSE IN DEPTH
 ...
 *
 * 6. TRACK ASSETS INTERNALLY TO PREVENT ATTACKS
 *    ✅ Don't rely on address(this).balance
 *    ✅ Track deposits/withdrawals separately
 *    ✅ Prevents donation attacks and accounting bugs
 *    ✅ Enables proper accounting verification
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                        COMMON MISTAKES
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * ❌ Using push payments for distributions (DoS risk!)
 * ❌ External calls before state updates (reentrancy)
 * ❌ Using .transfer() or .send() (breaks with smart contracts)
 * ❌ Not checking .call() return value (silent failures)
 * ❌ Relying on address(this).balance (donation attacks)
 * ❌ Unbounded loops over user arrays (gas limit DoS)
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                          NEXT STEPS
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * • Study OpenZeppelin PullPayment contract
 * • Explore withdrawal queue patterns
 * • Learn about batch withdrawal mechanisms
 * • Move to Project 13 to learn about block time logic
 */
