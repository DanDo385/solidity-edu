 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * 1. SOLIDITY 0.8.0 CHANGED EVERYTHING
 *    ✅ Pre-0.8: Arithmetic silently wraps on overflow/underflow
 *    ✅ Post-0.8: Arithmetic automatically reverts on overflow/underflow
 *    ✅ No SafeMath needed in 0.8+ (built-in protection)
 *    ✅ Real-world: Prevented countless exploits
 *
 * 2. OVERFLOW CAN CREATE INFINITE TOKENS
 *    ✅ uint256 max + 1 = 0 (wraps around)
 *    ✅ Attacker can mint max tokens from zero balance
 *    ✅ Real-world: BeautyChain BEC token exploit ($70M+)
 *    ✅ Batch operations especially vulnerable
 *
 * 3. UNDERFLOW CAN CREATE MAX VALUE
 *    ✅ 0 - 1 = 2^256 - 1 (huge number!)
 *    ✅ Attacker can drain tokens or create max balance
 *    ✅ Transfer from zero balance = underflow
 *    ✅ Real-world: Multiple token exploits before 0.8.0
 *
 * 4. UNCHECKED BLOCKS ARE POWERFUL BUT DANGEROUS
 *    ✅ Use for gas optimization when overflow is impossible
 *    ✅ Example: Loop counters, array indices
 *    ✅ Never use when overflow is possible
 *    ✅ Always document why unchecked is safe
 *
 * 5. SAFEMATH WAS THE PRE-0.8 SOLUTION
 *    ✅ Library that checked all arithmetic operations
 *    ✅ Added gas overhead but prevented exploits
 *    ✅ Still used in some 0.8+ projects for explicit checks
 *    ✅ No longer needed with automatic checks
 *
 * 6. HISTORICAL EXPLOITS WERE DEVASTATING
 *    ✅ BeautyChain BEC: Batch transfer overflow
 *    ✅ SMT Token: Transfer proxy overflow
 *    ✅ Multiple tokens: Underflow exploits
 *    ✅ Led to Solidity 0.8.0 automatic protection
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                        COMMON MISTAKES
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * ❌ Using unchecked blocks without careful analysis (overflow risk!)
 * ❌ Assuming arithmetic is always safe (pre-0.8 behavior!)
 * ❌ Not checking intermediate calculations (batch operations!)
 * ❌ Downcasting without validation (truncation bugs!)
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                          NEXT STEPS
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * • Study historical overflow exploits (BEC, SMT)
 * • Learn when unchecked blocks are safe
 * • Understand gas optimization vs security trade-offs
 * • Move to Project 33 to learn about MEV and front-running
 */
