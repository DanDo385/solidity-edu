 ...
 * ═══════════════════════════════════════════════════════════════════════════
 *
 * 1. DELEGATECALL EXECUTES IN CALLER'S CONTEXT
 *    ✅ Code runs from target contract
 *    ✅ Storage changes happen in caller's contract
 *    ✅ msg.sender and msg.value preserved
 *    ✅ Real-world: Used in proxy patterns (Project 10)
 *
 * 2. STORAGE LAYOUT COLLISIONS ARE DEVASTATING
 *    ✅ Proxy slot 0 = implementation address
 *    ✅ Implementation slot 0 = value (uint256)
 *    ✅ Delegatecall overwrites proxy's implementation!
 *    ✅ Attacker can become owner and control proxy
 *
 * 3. EIP-1967 PREVENTS STORAGE COLLISIONS
 *    ✅ Uses keccak256("eip1967.proxy.*") - 1 slots
 *    ✅ Unlikely to collide with implementation storage
 *    ✅ Standardized across all proxy implementations
 *    ✅ CONNECTION TO PROJECT 10: Same pattern!
 *
 * 4. CONSTRUCTORS DON'T WORK WITH DELEGATECALL
 *    ✅ Constructors execute during deployment
 *    ✅ Not included in runtime bytecode
 *    ✅ Use initializer functions instead
 *    ✅ Protect initializers from re-initialization
 *
 * 5. PARITY WALLET EXPLOIT WAS CATASTROPHIC
 *    ✅ Unprotected initialization function
 *    ✅ Shared library destroyed, froze $150M+
 *    ✅ Lesson: Always protect initialization
 *    ✅ Real-world: Led to better proxy patterns
 *
 * 6. PROXY PATTERNS REQUIRE EXTREME CARE
 *    ✅ Storage layout must match exactly
 *    ✅ Never change storage variable order
 *    ✅ Use EIP-1967 slots for proxy data
 ...
 * ❌ Using constructors in implementation (doesn't work!)
 * ❌ Changing storage layout in upgrades (corruption!)
 * ❌ Not protecting initialization functions
 * ❌ Custom proxy implementation instead of audited library
 *
 * ═══════════════════════════════════════════════════════════════════════════
 *                          NEXT STEPS
 * ═══════════════════════════════════════════════════════════════════════════
 ...
 * • Review Parity Wallet exploit analysis
 * • Learn about storage layout verification
 * • Move to Project 36 to learn about access control bugs
 */
