...
This project teaches you how to work with blockchain time and block properties in Solidity. You'll learn about the critical differences between `block.timestamp` and `block.number`, understand miner manipulation risks, and implement safe time-based logic patterns.

## Learning Objectives

- Understand `block.timestamp` vs `block.number`
- Learn about miner manipulation possibilities
- Implement rate limiting patterns
- Create cooldown mechanisms
- Recognize and avoid time-based exploits
- Use Foundry's time manipulation in tests (`vm.warp()`, `vm.roll()`)
