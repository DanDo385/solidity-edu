...
func main() {
	...
	for _, lg := range logs {
		...
		if _, err := db.Exec(`INSERT INTO transfers(block, txhash, sender, recipient, value) VALUES (?, ?, ?, ?, ?)`, lg.BlockNumber, lg.TxHash.Hex(), fromA.Hex(), toA.Hex(), data.Value.String()); err != nil {
			log.Printf("insert failed for block=%d tx=%s: %v", lg.BlockNumber, lg.TxHash.Hex(), err)
			continue
		}
