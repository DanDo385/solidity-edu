...
// EDUCATIONAL NOTES:
// - Nonce = ticket number for an address; txs must use sequential nonces or they stay pending.
// - Legacy gasPrice path; EIP-1559 (module 06) is recommended in production.
// - Signer uses chainID (EIP-155) to prevent cross-chain replay.
// - Analogy: post-office queue; your ticket (nonce) must be used in order.
// - Builds on keys (module 03) and RPC basics (01–02) to actually send value.
func main() {
    ...
    // Convert ETH float to wei (demo only; for precision use big.Rat/decimals!).
    ...
    // Warnings: do NOT use real keys on mainnet; prefer EIP-1559 fees; wait for receipt (module 15) to confirm.
}
